...
describe("Tests of Account routes", () => {
        ...
        describe("Login tests", () => {
            ...
            describe("Test that submitting the correct password logs you in and sends a token", () => {
                it("will respond with 200 and the users username and token.", async () => {
                    ...
                    let aTestUser = await User.findOne({ userName: newTestUser.username });
               
                    aTestUser.admin = true;
                    await aTestUser.save();
    ...
    describe("db down tests", () => {
        before(async () => {
            await database.close();
            
        })

        describe("new user with db down", () => {
            ...
            expect(response.body).to.have.property("message").that.includes("Client must be connected before runni");
            })
